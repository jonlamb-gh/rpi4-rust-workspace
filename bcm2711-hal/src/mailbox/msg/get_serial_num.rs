...
const TAG: TagId = TagId::GetSerialNum;
...
impl<T: AsRef<[u32]>> Resp<T> {
    ...
    #[inline]
    pub fn serial_number_low(&self) -> u32 {
        let data = self.buffer.as_ref();
        data[0]
    }

    #[inline]
    pub fn serial_number_high(&self) -> u32 {
        let data = self.buffer.as_ref();
        data[1]
    }
}
...
impl Repr {
    ...
    pub fn parse_response<T: AsRef<[u32]> + ?Sized>(msg: &Msg<&T>) -> Result<Repr> {
        ...
        let low = resp.serial_number_low() as u64;
        let high = resp.serial_number_high() as u64;

        Ok(Repr {
            serial_number: low | (high << 32),
        })
    }
